...
<form class="form-horizontal" method="post" id="flow_attribute" name="flow_attribute"
      action="{{route('process.update',['id'=>$process->id])}}">
    <div class="tab-content">
        <div class="tab-pane active" id="attrBasic">
            <div class="control-group">
                <label class="control-label" for="process_name">步骤名称</label>
                <div class="controls">
                    <input type="text" id="process_name" name="process_name" placeholder="步骤名称"
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <label class="radio inline">
                        <input type="radio" readonly name="process_position" value="1"
                               @if($process->position==1) checked="checked" @endif>正常步骤
                    </label>
                    @if($can_child)
                        <label class="radio inline">
                            <input type="radio" name="process_position" value="2"
                                   @if($process->position==2) checked="checked" @endif>转入子流程
                        </label>
                    @endif
                    <label class="radio inline">
                        <input type="radio" readonly name="process_position" value="0"
                               @if($process->position==0) checked="checked" @endif >第一步
                    </label>
                </div>
            </div>
            <hr/>
                ...
                <div class="offset1">
                    <select multiple="multiple" size="6" name="process_to[]" id="process_multiple">
                        @foreach($next_process as $v)
                            @if($v->next_process)
                                <option value="{{$v->next_process_id}}"
                                        selected="selected">{{$v->next_process->process_name}}</option>
                            @endif
                        @endforeach
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <select name="child_flow_id">
                            ...
                            @foreach($flows as $v)
                                <option value="{{$v->id}}"
                                        @if($process->child_flow_id==$v->id) selected="selected" @endif >{{$v->flow_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="controls">
                        <label class="radio inline">
                            <input type="radio" name="child_after" value="1"
                                   @if($process->child_after==1) checked @endif>
                            同时结束父流程
                        </label>
                        <label class="radio inline">
                            <input type="radio" name="child_after" value="2"
                                   @if($process->child_after==2) checked @endif>
                            返回父流程步骤
                        </label>
                    </div>
                </div>
                    ...
                    <div class="controls">
                        <select name="child_back_process">
                            ...
                            @foreach($processes as $v)
                                <option value="{{$v->id}}"
                                        @if($process->child_back_process==$v->id) selected="selected" @endif >{{$v->process_name}}</option>
                            @endforeach
                        </select>
        ...
        <div class="tab-pane" id="attrPower">
            <div class="control-group">
                ...
                <div id="auto_person_4" class="hide">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定主办人" name="auto_sponsor_uids"> <a
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定经办人" name="auto_respon_uids"> <a
                                    href="javascript:void(0);">选择</a>
                        </div>
                    </div>
                </div>
                <div id="auto_person_5" class="hide">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" placeholder="指定角色" name="auto_role_ids"> <a
                                    href="javascript:void(0);">选择</a>
                        </div>
                    </div>
                </div>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="hidden" name="range_emp_ids" id="range_emp_ids"
                           value="{{$select_emps->implode('id',',')}}">
                    <input class="input-xlarge" readonly="readonly" type="text" placeholder="选择人员" name="range_emp_text"
                           id="range_emp_text" value="{{$select_emps->implode('name',',')}}"> <a
                            href="javascript:void(0);" class="btn"
                            onclick="superDialog('/flowlink/auth/emp/{{$process->id}}','range_emp_text','range_emp_ids');">选择</a>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">授权部门</label>
                <div class="controls">
                    <input type="hidden" name="range_dept_ids" id="range_dept_ids"
                           value="{{$select_depts->implode('id',',')}}">
                    <input class="input-xlarge" readonly="readonly" type="text" placeholder="选择部门"
                           name="range_dept_text" id="range_dept_text"
                           value="{{$select_depts->implode('dept_name',',')}}"> <a href="javascript:void(0);"
                                                                                   class="btn"
                                                                                   onclick="superDialog('/flowlink/auth/dept/{{$process->id}}','range_dept_text','range_dept_ids');">选择</a>
                </div>
            </div>
            <div class="control-group hide">
                ...
                <div class="controls">
                    ...
                    <input class="input-xlarge" readonly="readonly" type="text" placeholder="选择角色"
                           name="range_role_text" id="range_role_text" value=""> <a href="javascript:void(0);"
                                                                                    class="btn"
                                                                                    onclick="superDialog('/flowlink/auth/role/{{$process->id}}','range_role_text','range_role_ids');">选择</a>
                </div>
            </div>
                ...
                <tr id="tpl" class="hide">
                    ...
                    <td>
                        <table class="table table-condensed">
                                ...
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-small"
                                                onclick="fnAddLeftParenthesis('@a')">（
                                        </button>
                                        <button type="button" class="btn btn-small"
                                                onclick="fnAddRightParenthesis('@a')">）
                                        </button>
                                        <button type="button" onclick="fnAddConditions('@a')" class="btn btn-small">新增
                                        </button>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <button type="button" onclick="fnClearCon('@a')" class="btn btn-small">清空
                                        </button>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <input id="process_in_desc_@a" type="text" name="process_in_desc_@a"
                                           style="width:98%;">
        ...
        <div class="tab-pane" id="attrStyle">
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="input-small" name="style_width" id="style_width" placeholder="宽度PX"
                           value="{{$process->style_width}}"> X <input type="text" class="input-small"
                                                                       name="style_height" id="style_height"
                                                                       placeholder="高度PX"
                                                                       value="{{$process->style_height}}">
                </div>
            </div>
            ...
            <div class="control-group">
                <label class="control-label" for="process_name"><span class="process-flag badge badge-inverse"><i
                                class="icon-star-empty icon-white" id="style_icon_preview"></i></span> 图标</label>
                <div class="controls">
                    <input type="text" class="input-medium" name="style_icon" id="style_icon" placeholder="icon"
                           value="{{$process->icon}}">
                    <div class="colors" org-bind="style_icon">
                        <ul>
                            <li class="Black active" org-data="icon-star-empty" title="Black"><i
                                        class="icon-star-empty icon-white"></i></li>
                            <li class="red" org-data="icon-ok" title="Red"><i class="icon-ok icon-white"></i></li>
                            <li class="green" org-data="icon-remove" title="Green"><i
                                        class="icon-remove icon-white"></i></li>
                            <li class="blue" org-data="icon-refresh" title="Blue"><i
                                        class="icon-refresh icon-white"></i></li>
                            <li class="aero" org-data="icon-plane" title="Aero"><i class="icon-plane icon-white"></i>
                            </li>
                            <li class="grey" org-data="icon-play" title="Grey"><i class="icon-play icon-white"></i></li>
                            <li class="orange" org-data="icon-heart" title="Orange"><i
                                        class="icon-heart icon-white"></i></li>
                            <li class="yellow" org-data="icon-random" title="Yellow"><i
                                        class="icon-random icon-white"></i></li>
                            <li class="pink" org-data="icon-home" title="Pink"><i class="icon-home icon-white"></i></li>
                            <li class="purple" org-data="icon-lock" title="Purple"><i class="icon-lock icon-white"></i>
                            </li>
                        </ul>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('#attributeOK').click(function () {
       if($('#auto_person_id').val() === "0"){
           // if($('#range_emp_text').val() === '' && $('#range_dept_text').val() === ''){
           //     layer.msg('当设置为自动选人时，请手动选择授权人员或者授权部门');
           //     return false;
           // }
       }
       return true;
    ...
    function fnAddConditions(id) {
        var sField = $('#field_' + id).val(), sField_text = $('#field_' + id).find('option:selected').text(),
            sCon = $('#condition_' + id).val(), sValue = $('#item_value_' + id).val();
                            ...
                            $.post(get_con_url, {
                                ...
                                $.each(data, function (i, n) {
                                    if (i == id && _id('conList_' + i)) {
                                        $('#conList_' + i).append("<option value='"+n.option+"' selected>"+n.desc+"</option>");
                                        // $('#conList_' + i).addOption([
                                        //     {
                                        //         name: n.desc,
                                        //         value: n.option,
                                        //         selected: true
