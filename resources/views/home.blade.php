                    ...
                    <div class="panel-body">
                        @foreach($flows as $v)
                            <a class="label label-danger"
                               href="{{route('entry.create',['flow_id'=>$v->id])}}">{{$v->flow_name}}</a>
                        @endforeach
                    </div>
                </div>
            </div>
                        ...
                        @if($entries->count()>0)
                            @foreach($entries as $v)
                                <tr>
                                    ...
                                    <td>
                                        @if($v->child>0)
                                            <span class="text text-danger">子流程({{$v->child_process->flow->flow_name}}):</span>
                                            {{$v->child_process->process_name}}
                                        @else
                                    ...
                                    <td>
                                        @if($v->status==-1)
                                            ...
                                            <a href="/entry/resend?entry_id={{$v->id}}"
                                               class="btn btn-xs btn-warning">重新发起</a>
                                            <a href="/entry/cancel?entry_id={{$v->id}}" class="btn btn-xs btn-info">撤销</a>
                                        @endif
                                        <button onclick="superDialog('/entry/{{$v->id}}','','');"
                                                class="btn btn-xs btn-info entry-detail">详情
                                        </button>
                                        <button onclick="superDialog('/proc?entry_id={{$v->id}}')"
                                                class="btn btn-xs btn-primary">进程明细
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
                            ...
                            <tr>
                                ...
                                @if($v->status==0)
                                    ...
                                    @elseif($v->status==-1)
                                        已处理(驳回)
                                    @endif
                                    <button onclick="superDialog('/proc?entry_id={{$v->entry->id}}')"
                                            class="btn btn-xs btn-primary">进程明细
                                    </button>
                                </td>
                            </tr>
            ...
            <div class="panel panel-success">
                ...
                @foreach($handle_procs as $v)
                    <div class="alert alert-info" role="alert">
                        <h3><i></i>{{$v->first()->entry->emp->name}}:{{$v->first()->entry->title}} (
                            <small><a href="javascript:;"
                                      onclick="superDialog('/proc?entry_id={{$v->first()->entry_id}}')">{{$v->count()}}</a>
                            </small>
                            )
                        </h3>
                        <ul>
                            @foreach($v as $v2)
                                <li>
                                    <p>
                                        <strong>@if($v2->status==9)<span
                                                    class="text text-success">通过</span>@elseif($v2->status==-1)<span
                                                    class="text text-danger">驳回</span>@endif
                                        </strong>
                                        <small>{{$v2->auditor_name}}</small>
                                        - {{$v2->process_name}} <span>{{$v2->created_at}}</span>
                                    </p>
                                </li>
                            @endforeach
                        </ul>
