@if(empty($entry_data))
    ...
    @foreach($template_forms as $v)
        ...
        @elseif($v->field_type=='textarea')
            <div class="form-group">
                <label>{{$v->field_name}}</label>
                <textarea rows="3" class="form-control" placeholder="{{$v->field_name}}"
                          name="tpl[{{$v->field}}]"></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="tpl[{{$v->field}}]">
                    ...
                    @foreach($options as $op)
                        <option value="{{$op}}"
                                @if($v->field_default_value==$op) selected="selected" @endif>{{$op}}</option>
                    @endforeach
                </select>
            </div>
    ...
    @foreach($template_forms as $v)
        ...
        @elseif($v->field_type=='textarea')
            <div class="form-group">
                <label>{{$v->field_name}}</label>
                <textarea rows="3" class="form-control" placeholder="{{$v->field_name}}"
                          name="tpl[{{$v->field}}]">{{$field_value}}</textarea>
            </div>
        @elseif($v->field_type=='date')
            <div class="form-group">
                <label>{{$v->field_name}}</label>
        ...
        @elseif($v->field_type=='checkbox')
            <div class="form-group">
                ...
                @foreach($checkbox as $c)
                    <label class="checkbox-inline">
                        <input type="checkbox" value="{{$c}}" name="tpl[{{$v->field}}][]"
                               @if(in_array($c,$field_arr)) checked="checked" @endif > {{$c}}
                    </label>
                @endforeach
            </div>
        @elseif($v->field_type=='radio')
            <div class="form-group">
                ...
                @foreach($radios as $r)
                    <label class="radio-inline">
                        <input type="radio" name="tpl[{{$v->field}}]" value="{{$r}}"
                               @if($field_value==$r) checked="checked" @endif> {{$r}}
                    </label>
                @endforeach
            </div>
        @elseif($v->field_type=='file')
